...
func TestHejsan(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			links, err := Parse(r)
			if err != nil {
				t.Errorf("Did not expect an error here: %v", err)
			}

			if len(links) != tt.exLen {
				t.Errorf("Expected the length of this slice to be %v but it was (%v)", tt.exLen, len(links))
			}

			for i, v := range tt.exTexts {
				text := links[i].Text
				if text != v {
					t.Errorf("Expected the text of this link to be %v but it was (%v)", v, text)
				}
			}
		})
	}
...
<html lang="en">
  <body>
    ...
    <a href="/one-more">Another Link</a>
  </body>
</html>`
