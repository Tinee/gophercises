...
func TestHejsan(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f, err := os.Open("./test-fixtures/first.html")
			if err != nil {
				t.Errorf("Error could not open file: %v", err)
			}
			p := New(f)
			links := p.Link()

			if len(links) != tt.expectedLen {
				t.Errorf("Expected the length of this slice to be %v but it was %v", tt.expectedLen, len(links))
			}
		})
	}
}
